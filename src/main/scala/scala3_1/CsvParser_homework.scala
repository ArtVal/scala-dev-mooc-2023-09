...
given booleanFieldConversion: FieldConversion[String, Boolean] with
  def convert(x: String): Boolean = x.toLowerCase.contains("true")
...
object TestExecution{
  ...
  def parse[String,B](x:String)(using fc: FieldConversion[String, B]) : B =
    fc.convert(x)
  ...
  def main(args: Array[String]): Unit = {
    ...
    def IntField =  StringField.map(str => parse[String, Int](str)) //StringField.map(...здесь применить parse который подхватит нужный given автоматически ...)
    def FloatField = StringField.map(str => parse[String, Float](str))
    def BooleanField = StringField.map(str => parse[String, Boolean](str))
    ...
    class ParserWithGivenParam(using splitter: String) {
      def parseCar(x: String): Seq[Car] =
        ...
        x.split(splitter).map(parser.parse)

    }

    object ParserWithGivenParam {
      def apply(using splitter: String) =
        new ParserWithGivenParam()
    }

    println(ParserWithGivenParam.apply.parseCar(str).map(x=>s"${x.model},${x.mark},${x.year}").mkString(";"))
  }
}